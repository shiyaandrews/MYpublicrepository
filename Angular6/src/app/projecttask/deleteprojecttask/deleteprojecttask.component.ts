...
export class DeleteprojecttaskComponent implements OnInit {
    ...
    this.http.get(`http://localhost:51634/api/ProjectTask/${this.id}`).subscribe(
      data => {
        ...
        this.formData=new FormGroup({
          ...
          task_end_date: new FormControl(this.projecttask.task_end_date,[]),
          task_start_date: new FormControl(this.projecttask.task_start_date,[]),
          task_completion: new FormControl(this.projecttask.task_completion,[]),
          user_story_id: new FormControl(this.projecttask.user_story_id,[])
        });
  ...
  onClickSubmit(projecttask){
    ...
    this.http.delete(`http://localhost:51634/api/ProjectTask/${this.id}`).subscribe(
      data => {
